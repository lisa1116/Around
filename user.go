...
func checkUser(username, password string) (bool, error) {
	...
	// if user does not exist, return false is fine, indicating server is functioning well, it's the client side problem
	return false, nil
}
...
func addUser(user *User) (bool, error) {
	...
	// if searchResult length is greater than 0, it means there is an existing user name , the new user cannot be added
...
// why 1 input parameters is pointer, the other one is not pointer? Because ResponseWriter is interface (interface data type cannot be initiated), the concept of pointer does not exist. But Request is a concrete data type
